<script>
  ...
  (function (exports) {
    // (c) 2014, Leon Bambrick, Doeke Zanstra
    console.clear();
    /*** IIFE start **********************************************************************************************/
    ('use strict');
    ...
    function start() {
      ...
      var info =
        "Type [Help] for help at any time (commands are provided between [], so don't type the brackets).";
    ...
    /*const*/
    var localStorageKey = 'console-log-data';
    var cmdHistory = [];
    var tellbuf = [];
    var movecount = 0,
      playerState;
    ...
    //         if (fnEach(array[index]))
    //           //only advance array, when it's OK
    //           index++;
    //       } else {
    //         fnFinal();
    ...
    function RegisterCommands(fnCommand, synonyms, hasSetter /*no load/save support for setter*/) {
      ...
      synonyms.split(',').forEach(function (cmd) {
        if (cmd in exports) {
          // console.error(
          //   '%cCommand %s is already defined',
          //   'font-size:150%;color:red;font-weight:bold',
          //   cmd,
          //   fnCommand,
          // );
        } else {
    ...
    function Help() {
      ...
      tell(
        'Also: [l] for [Look], [t] for [Take], [u] for [Use], [sp] for [Speak], [Eat], [i] for [Inventory],',
      );
      tell('[Me] for player state, [f] for [Fight] and [h] for [Help]. You can also [Reset], [Save]');
      tell('and [Load] or [Remove] a game.');
    }
    ...
    function Look() {
      ...
      switch (cell) {
        case 'H':
          return (
            tell(
              'The house is weathered, and appears abandoned. The front door is partially off its hinges.',
            ),
            tell('You could easily [Enter].')
          );
        case 'V':
          return tell(
            'Cool air blows from the cave. The entrance is large enough that you can [Enter]. You can move ' +
              Possibilities(),
          );
        case '2':
          return tell('There is a rope you can climb [Up]. It looks sturdy enough to hold your weight.');
        case 'k':
          return (
            tell('This kitchen contains one of those fancy Magic Amulet Receptacle Devices.'),
            tell('You could try to [Use] it, I guess.')
          );
        case 'r': {
          act(1000, [
        ...
        default:
          return tell('You see nothing else.');
      }
    }
      ...
      switch (cell) {
        case 'k': {
          if (playerInventory.amulet == 1) {
            ...
            return tell(
              "It's not clear how to use this Magic Amulet Receptacle Device. Perhaps it requires a magic amulet. But where would we get one of those?",
            );
          }
        }
        default:
          tell("You can't use that here.");
      }
    }
    ...
    function Load(value) {
      if (value) {
        ...
        //   timedLoad(
        //     -timeout,
        //     'eat,w,w,sp,e,e,s,s,l,ent,w,w,n,t,inv,s,e,e,n,l,up,n,n,n,n,w,w,w,w,w,n,w,w,s,s,f,n,n,e,e,n,e,e,e,e,e,e,e,s,e,e,s,t,inv,s,w,w,n,w,w,w,s,s,e,e,l,ent,w,n,l,ent,w,w,w,s,n,w,e,n,l,me,use'.split(
        //       ',',
        //     ),
        //   );
        tell("I know you think you should but really you shouldn't");
        return;
      }
    ...
    function Remove() {
      return tell(
        remove() ? 'The saved game has been deleted.' : 'You never seem to have saved your gameplay, Mister!',
      );
    }
    ...
    function move(x, y) {
      if (here[0] + x < 0 || here[0] + x > maps[mapnum][0].length)
        return tell('the edge of the world has been reached.');
      if (here[1] + y < 0 || here[1] + y > maps[mapnum].length)
        return tell('the edge of the world has been reached.');
    ...
    function say() {
      ...
      switch (cell) {
        case 'T':
          return tell('You are in a town square. Paths lead to the ' + Possibilities() + '.');
        case 'P':
          return tell('You walk carefully along the path. You can move ' + Possibilities() + '.');
        case 'H':
          return tell('There is a house.');
        case 'F':
          return tell('A spooky forest. You can proceed ' + Possibilities() + '.');
        case 'f':
          return tell("The forest is so thick, you don't know what directions are possible.");
        case 'E':
          return elfhut();
        case 'W':
          return tell('A stream with a waterfall blocks your way. It is teeming with fish.');
        case 'C':
          return tell('You are standing on a cliff by the sea. Seagulls are squawking.');
        case 'V':
          return tell('The entrance to a cave. Chills run up and down your spine.');
        case 'k':
          return tell('You are in a kitchen. You can move ' + Possibilities() + '.');
        case 'h':
          return tell('You are in a hallway. You can move ' + Possibilities() + '.');
        case 'r':
          return tell('You are in a bedroom. You can move ' + Possibilities() + '.');
        case 'b':
          return tell('You are in a bathroom. You can move ' + Possibilities() + '.');
        case 'y':
          return tell('You are in a foyer. You can move ' + Possibilities() + '.');
        case 'a':
          return tell('Stairs lead up to an attic. You can move [Up], ' + Possibilities() + '.');
        case '0':
          return tell(
            'You are in a deep dark tunnel. Feeling with your fingers you find you can move ' +
              Possibilities() +
              '.',
          );
        case '1':
          return (
            tell('Somehow it is even darker here. You feel yourself going cray cray.'),
            tell(
              'The one thing you still have is your sense of direction. You can move ' +
                Possibilities() +
                '.',
            )
          );
        case '2':
          return tell(
            'You are in a wide open cavern. Bones litter the floor. You can move ' + Possibilities() + '.',
          );
        case '3':
          return horde();
        case '4':
          return (
            die(
              250,
              'You fall forever.'.split('').map(function (s, i) {
                return repeat(' ', i) + s;
              }),
            ),
            tell('A bottomless pit.')
          );
        default:
          return tell('You are in a place i do not recognize.');
      }
    }
    ...
    function Up() {
      ...
      if (cell == 'a') {
        die(1000, [
          ...
          'The stairs collapse.',
          'The entire house falls on top of you.',
        ]);
        ...
        here = [9, 7];
        tell(
          "Slowly you climb the rope. It creaks and threatens to snap. Eventually you've hauled yourself to the surface.",
        );
    ...
    function Enter() {
      ...
      switch (cell) {
        ...
        default:
          return tell('That is not possible here.');
      }
      return '';
    }
    ...
    function replaceLast(x, what, replacement) {
      return x
        ...
        .reverse()
        .join('')
        .replace(new RegExp(what.split('').reverse().join('')), replacement.split('').reverse().join(''))
        .split('')
        .reverse()
        .join('');
    }
...
<ViewContainer>
  <div slot="head" class="flex justify-between">
    <PageHeader
      pageIcon="cow_med.svg"
      pageTitle="Secret Cow Level"
      pageSubtitle="Nothing here but us cows...."
    />
  </div>
  <ContainerWithHeader>
    <div slot="header" class="py-4 px-6 flex space-x-4">
      <p class="inline-block self-center">Omne Principium Difficile</p>
    </div>
    <div slot="body">
      <div class="flex justify-center my-4">
        <pre>> Welcome, traveller. Step forth and ponder the terminal to begin the journey.</pre>
      </div>
    </div>
  </ContainerWithHeader>
</ViewContainer>
