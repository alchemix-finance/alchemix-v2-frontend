...
<ContainerWithHeader>
  <div slot="header" class="py-4 px-4 flex space-x-4">
    <Button
      ...
      canToggle="{true}"
      selected="{operationMode === OperationModes.Borrow}"
      on:clicked="{() => {
        operationMode = OperationModes.Borrow;
      }}"
    >
      <img
        slot="leftSlot"
    ...
    <Button
      ...
      textColor="{$settings.invertColors ? 'bronze4' : 'white2'}"
      canToggle="{true}"
      selected="{operationMode === OperationModes.Repay}"
      on:clicked="{() => {
        operationMode = OperationModes.Repay;
      }}"
    >
      <img
        slot="leftSlot"
    ...
    <Button
      ...
      textColor="{$settings.invertColors ? 'bronze4' : 'white2'}"
      canToggle="{true}"
      selected="{operationMode === OperationModes.Liquidate}"
      on:clicked="{() => {
        operationMode = OperationModes.Liquidate;
      }}"
    >
      <img
        slot="leftSlot"
  ...
  <div slot="body">
    {#if operationMode === OperationModes.Borrow}
      <Borrow selectedVaults="{availableVaults}" />
