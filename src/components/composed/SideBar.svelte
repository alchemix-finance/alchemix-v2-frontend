...
<ul>
  {#each sidebarSetup as sidebarItem}
    <li
      class="p-4 rounded-xl mb-5 cursor-pointer flex justify-between transition-opacity {pathname.slice(1) === `${sidebarItem.path}` ? 'bg-grey10 opacity-100' : 'opacity-40'} hover:bg-grey10 hover:opacity-100"
      on:click='{() => routerGuard(sidebarItem.path)}'>
      <span>{$_(sidebarItem.label)}</span>
      <img src='images/icons/{sidebarItem.icon}'
           class='w-6 h-6' />
    </li>
  {/each}
</ul>
